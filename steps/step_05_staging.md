...
This step builds the STAGING layer between RAW and REPORTING. You will create:

- **Staging tables** with explicit DDL for materialised snapshots
- **Cleansing views** that apply transformations at query time (PII masking, enrichment, filtering)
- **Warehouse scaling** to demonstrate elastic compute
- **Zero-Copy Clone** for safe development copies
...
-- C2: SCALE UP — resize to MEDIUM (4x the compute of X-SMALL)
...
- **Views vs Tables** — views apply transformations at read time; tables store materialised snapshots
- **PII masking** — NI numbers and account numbers are masked in the staging views
- **Warehouse scaling** — resize compute on demand with ALTER WAREHOUSE; scale back down when done
- **Zero-Copy Clone** — instant copy of a table with no additional storage until data diverges
